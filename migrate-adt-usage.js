...
const MIGRATION_RULES = [
  ...
  // Pattern matching migrations (these should be the same, but ensure correct imports)
  {
    name: 'Update matchMaybe imports',
    find: /import\s*{\s*[^}]*matchMaybe[^}]*}\s*from\s*['"`]([^'"`]*)['"`]/gi,
    replace: (match, modulePath) => {
      return match.replace(modulePath, './fp-maybe-unified');
    }
  },
  {
    name: 'Update matchEither imports',
    find: /import\s*{\s*[^}]*matchEither[^}]*}\s*from\s*['"`]([^'"`]*)['"`]/gi,
    replace: (match, modulePath) => {
      return match.replace(modulePath, './fp-either-unified');
    }
  },
  {
    name: 'Update matchResult imports',
    find: /import\s*{\s*[^}]*matchResult[^}]*}\s*from\s*['"`]([^'"`]*)['"`]/gi,
    replace: (match, modulePath) => {
      return match.replace(modulePath, './fp-result-unified');
    }
  }
];
...
function shouldExcludeFile(filePath) {
  return EXCLUDE_PATTERNS.some(pattern => {
    if (pattern.includes('**')) {
      ...
      return glob.sync(pattern).some(excludedPath => 
        path.resolve(filePath) === path.resolve(excludedPath)
      );
    }
...
function findFiles() {
  ...
  INCLUDE_PATTERNS.forEach(pattern => {
    const matches = glob.sync(pattern, { ignore: EXCLUDE_PATTERNS });
    files.push(...matches);
  });
...
function migrateADTUsage() {
  ...
  const filesToProcess = files.filter(file => !shouldExcludeFile(file));
  console.log(`Processing ${filesToProcess.length} files (excluding ${files.length - filesToProcess.length})\n`);
...
function verifyMigration() {
  ...
  const files = findFiles().filter(file => !shouldExcludeFile(file));
